...
import { 
  predictMultiZoneDemand, 
  generateSyntheticData,
  detectAnomalies 
} from '@/utils/demandPredictor';
import { 
  TrendingUp, 
  TrendingDown, 
  Minus, 
...
// Campus zones with coordinates
export const CAMPUS_ZONES = [
  {
    zone: "B Block Boys Hostel (BMSIT)",
    lat: 13.13543,
    lng: 77.56668,
  },
  {
    zone: "BMSIT Boys Hostel (2nd Pin)",
    lat: 13.13423,
    lng: 77.56938,
  },
  {
    zone: "BMSIT Girls Hostel",
    lat: 13.10646,
    lng: 77.57173,
  },
  {
    zone: "BMSIT Main Campus",
    lat: 13.13440,
    lng: 77.56811,
  },
  {
    zone: "BMSIT Lab Block",
    lat: 13.13401,
    lng: 77.56855,
  },
];
...
export default function AdminHeatmap() {
  ...
  const loadDemandData = async () => {
    ...
    try {
      ...
      if (useRealData) {
        ...
        const rides = await getHistoricalRideData(30);
        
        // Convert rides to time series data
        historicalData = rides.map(ride => ({
          timestamp: ride.requestTime,
          value: 1,
          zone: ride.pickup
        }));

        // If no real data, fall back to synthetic
        if (historicalData.length === 0) {
          console.log('No real data found, using synthetic data');
          historicalData = generateSyntheticData(
      ...
      // Predict demand for each zone
      const hoursAhead = timeRange === '1h' ? 1 : 24;
      const zonePredictions = predictMultiZoneDemand(
        historicalData,
        CAMPUS_ZONES.map(z => z.zone),
        hoursAhead
      );

      // Merge with zone coordinates
      const enhancedPredictions = zonePredictions.map(pred => {
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case 'increasing': return <TrendingUp className="h-4 w-4 text-warning" />;
  ...
  const getCircleColor = (demand: number) => {
    if (demand > 60) return '#ef4444'; // red
    if (demand > 40) return '#f59e0b'; // yellow
    return '#10b981'; // green
  };

  const getCircleRadius = (demand: number) => {
    return Math.max(200, demand * 10); // minimum 200m, scales with demand
  };
  ...
  const mapCenter = predictions.length > 0
    ? {
        lat: predictions.reduce((sum, p) => sum + p.lat, 0) / predictions.length,
        lng: predictions.reduce((sum, p) => sum + p.lng, 0) / predictions.length
      }
    : { lat: 13.13440, lng: 77.56811 };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              ML-powered predictive analytics for transport demand
            </p>
          </div>
        ...
        <div className="grid md:grid-cols-4 gap-4">
          ...
          <Card className="p-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                {highDemandZones > 3 ? 'Consider extra vehicles' : 'Normal capacity'}
              </p>
            </div>
          </Card>
          ...
          <Card className="p-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                {anomalyZones > 0 ? 'Unusual patterns found' : 'Normal patterns'}
              </p>
            </div>
          </Card>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold flex items-center gap-2">
              ...
              <span className="px-2 py-1 bg-secondary rounded text-xs font-normal">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {loading ? (
            ...
            predictions
              ...
              .map((zone) => {
                const demandLevel = zone.predictedDemand > 60 
                  ? 'high' 
                        ...
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all ${
                              demandLevel === 'high' ? 'bg-destructive' :
                              demandLevel === 'medium' ? 'bg-warning' :
                              'bg-success'
                            }`}
                            style={{ 
                              width: `${Math.min((zone.predictedDemand / 100) * 100, 100)}%` 
                            }}
                          />
                        </div>
